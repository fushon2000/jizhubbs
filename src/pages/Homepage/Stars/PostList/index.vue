<template>
  <div class="posting-list">
    <ul class="posting-list-ul">
      <li v-for="post in postPage.records">
        <div class="posting-info">
          <h3 class="posting-title">
            <a href="#">{{ post.title }}</a>
            <span class="posting-noAnonymityComment">{{ post.noAnonymityComment === 1 ? '实名' : '匿名' }}</span>
          </h3>
    ...
    <div class="pagination">
      <el-pagination
          ...
          @current-change="getStarListByUid">
      </el-pagination>
    </div>
  </div>
</template>
...
import {getStarListByUid} from "@/api/posting";
...
export default {
  ...
  methods: {
    getStarListByUid(page) {
      getStarListByUid(this.$store.state.user.userInfo.uid, page).then(({data}) => {
        this.postPage = data
      })
    }
  },
  created() {
    this.getStarListByUid(this.page)
  },
}
</script>
