<template>
  <div class="create-posting-package">
...
<script>
import $ from "jquery";
import {createPostingPackage} from "@/api/posting";

export default {
  name: "CreatePostingPackage",
  data() {
    return {
      postingPackage: {
        name: '',
  ...
  methods: {
    back() {
      this.$bus.emit("changeShowCreate", false)
    },
    create() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          createPostingPackage(this.postingPackage).then(res => {
            this.$bus.emit("changeShowCreate", false, res.data)
            this.$message.success("创建成功O(∩_∩)O~")
          })
        } else {
