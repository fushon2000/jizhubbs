...
export default {
  ...
  mounted() {
    ...
    for (let i = 0; i < this.loadingMessage.length; i++) {
      this.$refs.loading.children[i].style.cssText = "animation:jump 2s " + (i - 1) * 100 + "ms linear infinite"
    }
    // jQuery 实现触底加载更多
    // 添加滚动事件
    // 滚动一次请求页数+1 每页4条数据
  ...
  beforeDestroy() {
    console.log('Recommend组件即将被销毁')
    this.tmpStar = {}
    // console.log(this.unfinished)
    // // 组件切换后会卡bug，若定时器还没进行完就切换组件，则会在另一个组件显示特效
    // this.unfinished.forEach(index=>{
    //   console.log(index)
    //   $(`#up${index} .dot-box`).css("display","none")
    //   $(`#up${index} .thumbs-up-box`).removeClass("dotFlyUp")
    //   $(`#up${index} .thumbs-up-box`).removeClass("dotFlyDown")
